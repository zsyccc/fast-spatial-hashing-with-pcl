...
PointNormalT::PointNormalT() : assigned(false) {}
PointNormalT::PointNormalT(const PointNormalT& p) {
    pcl::copyPoint(p, *this);
    assigned = p.assigned;
}

PointNormalT::PointNormalT(const pcl::PointNormal& p, bool f) {
    pcl::copyPoint(p, *this);
    assigned = f;
}

PointNormalT::operator pcl::PointNormal() {
    pcl::PointNormal ret;
...
bool VSA<PointT>::metric2_proxy_normal(const std::vector<int>& region,
...
template <typename PointT>
float VSA<PointT>::error_metric(const PointNormalT& point,
...
struct VSA<PointT>::PQElement {
    ...
    float error;
    const VSA* vsa;
    PQElement(int id, int t, const VSA* _vsa) : index(id), tag(t), vsa(_vsa) {
        error = vsa->error_metric(vsa->cloud->points[index], vsa->proxies[tag]);
    }
...
template <typename PointT>
void VSA<PointT>::setInputCloud(
    const typename pcl::PointCloud<PointT>::Ptr in) {
    pcl::copyPointCloud(*in, *cloud);
}

template <typename PointT>
void VSA<PointT>::setMetricOption(int option) {
    this->metric_option = option;
}

template <typename PointT>
void VSA<PointT>::setEps(float eps) {
    this->eps = eps;
}
...
template <typename PointT>
std::vector<std::vector<int>> VSA<PointT>::compute() {
    ...
    while (fabs(total_error_new - total_error) > eps) {
        ...
        for (int i = 0; i < k; i++) {
            float smallest_error =
                error_metric(cloud->points[cur_partition[i][0]], proxies[i]);
